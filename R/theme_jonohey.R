...
theme_card <- function(
  ...
  if (is.null(panel_fill)) panel_fill <- bg
  if (is.null(plot_bg_fill))  plot_bg_fill  <- bg
...
theme_axes_wiggle <- function(
  ...
  ggplot2::theme_minimal(base_size = base_size, base_family = base_family) +
    ggplot2::theme(
      ...
      axis.line        = elementalist::element_line_wiggle(amount = amount, n = wiggle_n, lineend = "round",
                                                           linewidth = linewidth,
                                                           colour = line_color),
      # breathing room
