...
knitr::opts_chunk$set(
  ...
  fig.height = 6,     # adjust to your patchwork aspect
  dpi        = 150,
  fig.align  = "center",
  out.width  = "100%",
  fig.retina = 2
...
You can install the development version of jonohey from [GitHub](https://github.com/) with:
...
# needs ggplot2 and ggbeeswarm
set.seed(42)

make_beeswarm_data6 <- function(n_per_group = 60) {
  g <- factor(rep(LETTERS[1:6], each = n_per_group), levels = LETTERS[1:6])
  idx <- as.integer(g)
  # Funny backstory: groups trained at 6 different altitudes
  # Higher letter means higher altitude, so higher scores on average
  y <- 10 + 2.2 * idx + rnorm(length(idx), sd = 1.8)   # clear increasing pattern
